...
class GlobalChatCtr {
  ...
  CollectionReference<Map<String, dynamic>> initializeChat(String roomId) {
    _roomInstant = FirebaseFirestore.instance
  ...
  void sendMessage({required String from, required String content}) {
    _roomInstant.add(
        {"from": from, 'content': content, 'date': DateTime.now().toString()});
  }
  ...
  Future createRoom(String chatname, String userid) async {
    ...
    FirebaseFirestore.instance
        ...
        .add({"chatid": chatid, "name": chatname});
  }

  Future<List<QueryDocumentSnapshot>> getGlobalChat() async {
    var res =
        await FirebaseFirestore.instance.collection('globalchatrooms').get();
  ...
  Future<String> GenerateToken(String channel) async {
    try {
      var res = await http.post(Uri.parse('http://10.0.2.2:3000/'),
          body: {"channelName": channel, "uid": "0", "tokenType": "uid"});
